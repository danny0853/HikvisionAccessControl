...
public interface HCNetSDK extends StdCallLibrary {
	...
	HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(HikvisionConnectionConfig.dllPath, HCNetSDK.class);
	/*** 宏定义 ***/
	// 常量
...
//DVR设备参数
	...
	public static class NET_DVR_DEVICECFG_V40 extends Structure {
		...
		public short wDevClass; // 设备大类备是属于哪个产品线，0 保留，1-50 DVR，51-100 DVS，101-150 NVR，151-200 IPC，65534
...
//通道图象结构
	...
	public static class NET_DVR_VILOST_V40 extends Structure {
		...
		public int[] dwRelAlarmOut = new int[MAX_ALARMOUT_V40]; /*
																 * 触发报警输出号，按值表示,采用紧凑型排列，从下标0 - dwRelAlarmOut
	...
	public static class NET_DVR_MOTION_SINGLE_AREA extends Structure {
		public byte[] byMotionScope = new byte[64
				* 96]; /* 侦测区域,0-96位,表示64行,共有96*64个小宏块,目前有效的是22*18,为1表示是移动侦测区域,0-表示不是 */
		public byte byMotionSensitive; /* 移动侦测灵敏度, 0 - 5,越高越灵敏,0xff关闭 */
		public byte[] byRes = new byte[3];
	}
	...
	public static class NET_DVR_MOTION_V40 extends Structure {
		...
		/* 0x1000: 抓图上传到云 */
		public int dwMaxRelAlarmOutChanNum; // 触发的报警输出通道数（只读）最大支持数量
		public int[] dwRelAlarmOut = new int[MAX_ALARMOUT_V40]; // 实际触发的报警输出号，按值表示,采用紧凑型排列，从下标0 - dwRelAlarmOut
																// -1有效，如果中间遇到0xffffffff,则后续无效
		public NET_DVR_SCHEDTIMEWEEK[] struAlarmTime = new NET_DVR_SCHEDTIMEWEEK[MAX_DAYS]; /* 布防时间 */
		/* 触发的录像通道 */
		public int dwMaxRecordChanNum; // 设备支持的最大关联录像通道数-只读
		public int[] dwRelRecordChan = new int[MAX_CHANNUM_V40]; /*
																	 * 实际触发录像通道，按值表示,采用紧凑型排列，从下标0 - dwRelRecordChan
																	 * -1有效，如果中间遇到0xffffffff,则后续无效
																	 */
	...
	public static class NET_DVR_PICCFG_V40 extends Structure {
		...
		public byte byFontSize; // 16*16(中)/8*16(英)，1-32*32(中)/16*32(英)，2-64*64(中)/32*64(英) 3-48*48(中)/24*48(英)
								// 4-24*24(中)/12*24(英) 5-96*96(中)/48*96(英) 6-128*128(中)/64*128(英)
								// 7-80*80(中)/40*80(英) 8-112*112(中)/56*112(英) 0xff-自适应(adaptive)
...
//IPC接入参数配置
	...
	public class NET_DVR_IPDEVINFO_V31 extends Structure {
		 ...
		 * Arrays.asList("byEnable","byProType","byEnableQuickAdd","byRes1","sUserName",
		 * "sPassword","byDomain","struIP","wDVRPort","szDeviceID","byRes2"); }
		 */
	}
...
//图片查找条件
	public static class NET_DVR_FIND_PICTURE_PARAM extends Structure {
		 ...
		 * 15-吉，16-苏，17-赣，18-辽，19-蒙，20-宁，21-青， 22-鲁，23-晋，24-陕，25-沪，26-川，27-台，28-津，
...
//查找结果结构体
	public static class NET_DVR_FIND_PICTURE_V40 extends Structure {
		...
		public byte byFileType; // 文件类型， :0定时抓图1 移动侦测抓图 2 报警抓图3 报警 | 移动侦测抓图 4 报警 & 移动侦测抓图 6 手动抓图 ,9-智能图片,10-
								// PIR报警，11- 无线报警，12- 呼救报警, 0xa 预览时截图，0xd 人脸侦测, 0xe 越界侦测，0xf 入侵区域侦测，0x10 场景变更侦测,
								// 0x11-设备本地回放时截图, 0x12-智能侦测, 0x32-防区报警, 0x33-紧急求助, 0x34-业务咨询
...
//穿越警戒面参数
	...
	public static class NET_VCA_INTRUSION extends Structure {
		...
		/*
		 * 检测目标，可支持多选，具体定义为： 0~所有目标（表示不锁定检测目标，所有目标都将进行检测） 0x01 ~ 人， 0x02 ~ 车， 0x04 ~ 其他
		 */
...
// 道闸控制
	public static class NET_DVR_BARRIERGATE_CFG extends Structure {
		 ...
		 * 若新平台支持byUnlock字段，需byUnlock字段赋值为1，并结合4~解锁道闸来进行解锁。byUnlock字段赋值为1后，“0-关闭道闸,1-
		 * 开启道闸,2-停止道闸”操作将不可用于解锁。
		 */
...
//获取交通数据条件结构 
	public static class NET_DVR_TRAFFIC_DATA_QUERY_COND extends Structure {
		 ...
		 * Bit6-车辆类型有效 Bit7-车辆品牌有效 Bit8-车道号有效 Bit9-监测方向有效 Bit10-最低速度有效 Bit11-最高速度有效
		 * Bit12-数据类型有效 Bit13-布控方式类型有效 Bit14-违法取证有效 Bit15-事件类型有效 Bit16-取证类型有效
		 */
		...
		/*
		 * Bit0-未知（其他） Bit1-白色 Bit2-银色 Bit3-灰色 Bit4-黑色 Bit5-红色 Bit6-深蓝色 Bit7-蓝色 Bit8-黄色
		 * Bit9-绿色 Bit10-棕色 Bit11-粉色 Bit12-紫色 Bit13-深灰色
		 */
		public int dwVehicleColor;// 车身颜色（支持按位表示，可以复选）
		/*
		 * Bit0-未知（其他） Bit1-客车 Bit2-大货车 Bit3-轿车 Bit4-面包车 Bit5-小货车 Bit6-行人 Bit7-二轮车
		 * Bit8-三轮车 Bit9-SUV/MPV Bit10-中型客车
		 */
		public int dwVehicleType;// 车辆类型（支持按位表示，可以复选）
		/**
		 * Bit0-其他（保留） Bit1-低速 Bit2-超速 Bit3-逆行 Bit4-闯红灯 Bit5-压车道线 Bit6-不按导向 Bit7-路口滞留
		 * Bit8-机占非 Bit9-违法变道 Bit10-不按车道 Bit11-违反禁令 Bit12-路口停车 Bit13-绿灯停车 Bit14-未礼让行人
		 * Bit15-违章停车 Bit16-违章掉头 Bit17-占用应急车道 Bit18-未系安全带
		 */
		public int dwIllegalType;
		/**
		 * Bit0-其他（保留） Bit1-拥堵 Bit2-停车 Bit3-逆行 Bit4-行人 Bit5-抛洒物 Bit6-烟雾 Bit7-压线 Bit8-黑名单
		 * Bit9-超速 Bit10-变道 Bit11-掉头 Bit12-机占非 Bit13-加塞
		 */
		public int dwEventType;
		/**
...
//交通数据结构体
	...
	public static class NET_DVR_TRAFFIC_DATA_QUERY_RESULT extends Structure {
		...
		/*
		 * Bit0-未知（其他） Bit1-客车 Bit2-大货车 Bit3-轿车 Bit4-面包车 Bit5-小货车 Bit6-行人 Bit7-二轮车
		 * Bit8-三轮车 Bit9-SUV/MPV Bit10-中型客车 Bit11-机动车 Bit12-非机动车 Bit13-小型轿车 Bit14-微型轿车
		 * Bit15-皮卡车 Bit16-集装箱卡车 Bit17-微卡，栏板卡 Bit18-渣土车 Bit19-吊车，工程车 Bit20-油罐车
		 * Bit21-混凝土搅拌车 Bit22-平板拖车 Bit23-两厢轿车 Bit24-三厢轿车 Bit25-轿跑 Bit26-小型客车
		 */
		...
		public NET_DVR_TRAFFIC_PICTURE_PARAM[] struTrafficPic = new NET_DVR_TRAFFIC_PICTURE_PARAM[MAX_TRAFFIC_PICTURE_NUM/*
																															 * 8
																															 */];
	}
...
//卡参数
	...
	public static class NET_DVR_CARD_CFG_V50 extends Structure {
		...
		public byte byCardModelType; // 0-空，1- MIFARE S50，2- MIFARE S70，3- FM1208 CPU卡，4- FM1216
...
//门禁主机事件信息
	...
	public static class NET_DVR_ACS_EVENT_DETAIL extends Structure {
		...
		public byte byType; // 防区类型，0:即时防区,1-24小时防区,2-延时防区 ,3-内部防区，4-钥匙防区 5-火警防区 6-周界防区 7-24小时无声防区
...
//统计信息
	...
	public static class NET_DVR_FOCUSMODE_CFG extends Structure {
		...
		public int dwFocusPos;/*
								 * dwFocusPos 是focus值（聚焦值），范围为[0x1000,0xC000]，这个值是sony坐标值，使用这个值是为了对外统一，
								 * 保证不同的镜头对外focus值都转换在这个范围内 (手动聚焦模式下下应用)
								 */
	...
	public static class NET_DVR_INQUEST_CDRW_STATUS extends Structure {
		/*
		 * 运行状态：0-审讯开始， 1-审讯过程中刻录，2-审讯停止， 3-刻录审讯文件, 4-备份(事后备份和本地备份) 5-空闲 6-初始化硬盘 7-恢复审讯
		 */
		public int dwType;
	...
	public static class NET_DVR_INQUEST_CDRW extends Structure {
		...
		public int dwStatus; /*
								 ...
								 * 当dwType=4时，0-刻录正常，1-无光盘或光盘异常，2-光盘已封盘(81不支持)，3-光盘空间不足 当dwType=5时,0-光盘正常，
								 * 1-无光盘或光盘异常,2-光盘已封盘(81不支持) 当dwType=6或7时, 0-刻录正常, 1-无光盘或光盘异常, 2-光盘已封盘(81不支持),
								 * 3-光盘空间不足
								 */
...
//物理磁盘 
	public static class NET_DVR_PHY_DISK_INFO extends Structure {
		...
		public byte byStatus; // 硬盘状态； 0-正常 1-降级 2-已删除 3-磁盘丢失 4-下线 5-次正常 6-外来 7-异常 8-SMART状态异常 9-休眠 10-有坏块
								// 0xff-不存在
...
//牌识参数
	public static class NET_ITC_PLATE_RECOG_PARAM extends Structure {
		...
		/*
		 * 识别的类型， bit0-背向识别：0-正向车牌识别，1-背向识别(尾牌识别) ； bit1-大车牌识别或小车牌识别：0-小车牌识别，1-大车牌识别 ；
		 * bit2-车身颜色识别：0-不采用车身颜色识别，在背向识别或小车牌识别时禁止启用，1-车身颜色识别；
		 * bit3-农用车识别：0-不采用农用车识别，1-农用车识别； bit4-模糊识别：0-不采用模糊识别，1-模糊识别；
		 * bit5-帧定位或场定位：0-帧定位，1-场定位； bit6-帧识别或场识别：0-帧识别，1-场识别； bit7-晚上或白天：0-白天，1-晚上
		 * bit8-摩托车识别：0-不采用摩托车识别，1-摩托车识别; bit9-场景模式：0-电警/多帧，1-卡口；
		 * bit10-微小车牌：0-不启用，1-启用微小车牌识别(像素60～80) bit11-安全带检测：0-不启用，1-启用安全带检测
		 * bit12-民航车牌识别: 0-不启用，1-开启民航车牌识别 bit13-车牌过渡倾斜处理: 0-不启用，1-开启过渡倾斜处理（PRS）
		 * bit14-超大车牌识别: 0-不启用，1-开启超大车牌识别（PRS） bit15-遮阳板检测：0-不启用，1-启用遮阳板检测
